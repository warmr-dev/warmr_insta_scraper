...
Лиды идут в Slack, а в Telegram — сообщения о том, что система встала.
...
Приходят при: истёкших куки, отсутствии активных аккаунтов, трёх падениях цикла
подряд. Одно и то же состояние не чаще раза в час — цикл идёт каждую минуту,
без этого был бы спам.

Главный случай — истёкшие куки: продлить их из кода нельзя, и без уведомления
простой заметят только по пропавшим лидам.

### Куки: три формата

`web-add` понимает всё, что отдаёт браузер:

```bash
# JSON от расширения-экспортёра (папка cookies/ в .gitignore)
... web-add yrsayl7 --cookies-file cookies/www_instagram_com_cookies.json

# строка из DevTools
... web-add yrsayl7 --cookies-file cookies.txt
```
