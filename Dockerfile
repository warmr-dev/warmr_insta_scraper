...
# Миграции по умолчанию НЕ запускаются: на Railway `alembic upgrade head` падал
# с "Can't locate revision '0002'" (база новее образа) и контейнер уходил в
# цикл падений. Схема меняется редко - применяйте вручную.
# RUN_MIGRATIONS=true включает миграцию при старте, но её сбой не остановит
# запуск.
