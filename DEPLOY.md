...
Redis не нужен: цикл работает последовательно, без очередей. `USE_REDIS=false`
стоит по умолчанию — именно попытка подключиться к несуществующему Redis роняла
контейнер на Railway.
...
Отключение происходит **само**: при `LoginRequiredError` аккаунт получает
`is_active=false` и причину в `last_error`, а следующие циклы его пропускают —
мёртвой сессией Instagram больше не беспокоим.

Важное различие: временные сбои (сеть, `429`) аккаунт **не** отключают. Они
пишут причину в `last_error`, но `is_active` остаётся `true` — иначе разрыв
связи на секунду выводил бы из строя рабочий аккаунт.

Лечение: зайти в браузер, скопировать семь куки, обновить строку в Supabase
(или `web-add` заново). `save_cookies` сам возвращает `is_active=true`, так что
аккаунт оживает без дополнительных действий.
